...
import { createTextTo3DTask } from "../_shared/meshy.ts";
...
function buildPreviewPayload(requestPayload: Record<string, unknown> | null) {
  ...
  return {
    mode: "preview",
    prompt,
    negative_prompt: typeof preview.negative_prompt === "string" ? preview.negative_prompt : undefined,
    art_style: typeof preview.art_style === "string" ? preview.art_style : "realistic",
    ai_model: typeof preview.ai_model === "string" ? preview.ai_model : undefined,
    topology: typeof preview.topology === "string" ? preview.topology : undefined,
    target_polycount: typeof preview.target_polycount === "number" ? preview.target_polycount : undefined,
    should_remesh: typeof preview.should_remesh === "boolean" ? preview.should_remesh : undefined,
    symmetry_mode: typeof preview.symmetry_mode === "string" ? preview.symmetry_mode : undefined,
    moderation: typeof preview.moderation === "boolean" ? preview.moderation : undefined,
    seed: typeof preview.seed === "number" ? preview.seed : undefined
  };
}
...
serve(async (req) => {
  ...
  try {
    ...
    const taskId = await createTextTo3DTask(previewPayload, meshyApiKey);
    const resultPayload = {
      ...(job.result_payload ?? {}),
    ...
    const message = err instanceof Error ? err.message : "Meshy preview failed";
    await admin
      .from("ai_jobs")
